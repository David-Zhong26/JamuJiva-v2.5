...
const TermsPage: React.FC = () => {
  const headerTitle = blocks[0]?.text ?? 'Website Terms of Use';
  const headerVersion = blocks[1]?.text ?? 'Version 1.0';
  const headerDate = blocks[2]?.text ?? '';

  const bodyBlocks = blocks.slice(3);

  return (
    <main className="bg-[#F5E8CA] pt-24 pb-16 md:pt-32 md:pb-24">
      <article className="mx-auto max-w-[680px] px-5 md:px-8">
        <header className="mb-8 text-center">
          <h1 className="text-lg font-bold text-[#2D4F3E] md:text-xl">{headerTitle}</h1>
          <p className="mt-1 text-sm font-bold text-[#2D4F3E]">{headerVersion}</p>
          <p className="text-sm font-bold text-[#2D4F3E]">{headerDate}</p>
        </header>
        ...
        <div className="space-y-4 text-[13px] leading-[1.65] text-[#2D4F3E] md:text-[14px]">
          {bodyBlocks.map((block, index) => {
            ...
            if (isShortSectionHeading(block)) {
              return (
                <h2 key={key} className="pt-2 text-sm font-bold text-[#2D4F3E] md:text-[14px]">
                  {block.text}
                </h2>
              );
            }
