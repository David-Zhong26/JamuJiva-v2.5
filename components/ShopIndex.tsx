      ...
      <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {SHOP_GROUPS.map((group) => (
          <Link
            key={group.id}
            to={`/shop/${group.slug}`}
            className="group relative overflow-hidden rounded-2xl border border-[#2D4F3E]/15 bg-[#F9EFD4]/40 transition-colors duration-300 hover:border-[#2D4F3E]/35 hover:bg-[#F4D8AE]"
          >
            {group.badge ? (
            ...
            <div className="flex min-h-[200px] items-center justify-center bg-[#F5E8CA] p-8 transition-colors duration-300 group-hover:bg-[#F2D2A2]">
              <img
                src={group.image}
                alt={group.name}
            ...
            <div className="p-6 transition-colors duration-300 group-hover:bg-[#F4D8AE]">
